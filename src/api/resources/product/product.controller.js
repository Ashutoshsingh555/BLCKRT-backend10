...
/* Add user api start here................................*/
     ...
     async addProduct(req, res, next) {
        try {
            ...
            const { name, description, photo, originPlace, aboutProduct, nutrientsDetals,storeUses, tags, lableType,reccomendedProduct, isTex,GSTrate,GSTtyp, HSNcode, categoryId, subCategoryId,  videoUpload,} = req.body;
                ...
                where: { name: name }
                    ...
                    if (!product) {
                        return db.product.create({
                            ...
                            nutrientsDetals:nutrientsDetals,
                            storeUses:storeUses,
                            lableType:lableType,
                            isTex:isTex,
                             ...
                             if(req.files){
                                ...
                                for (let i=0; i < reccomendedProduct.length; i++) {
                                       db.reccomendProduct.create({
                                        ...
                                        reccomendedId:reccomendedProduct[i]
    ...
    async getAllProductList2(req, res, next) {
        let limit = 200;
        let sort = ['id'];
        let offset = 0;
        let page = 1;
    ...
    async getAllProductList(req, res, next) {
     try {
           await db.product.findAll({
             include: [{ model: db.productphoto, attributes: ["id", "imgUrl"] },{ model: db.varientModel}, { model: db.tagModel, attributes: ["id","name"]},  {model:db.featureSettings,attributes:["recomendedProduct","deliverySlotStatus","loyaltyProgram","COD","showCOD","reccemendProductNo"]},  { model: db.reccomendProduct, attributes: ["id", "productName","reccomendedId"]}, { model: db.category, attributes: ["id", "name","photo"]},{ model: db.SubCategory, attributes: ["id", "sub_name" ,"photo"] }]
            }).then(product => {
                    res.status(200).json({ 'success': true, product });
                })
                .catch(function (err) {
      ...
      async getrecentandsearch(req, res, next) {
     try {
           if(req.body.search){
            db.RecentSearches.create({
        ...
        try{
           await  db.RecentSearches.destroy({where:({id:req.query.id})})
      ...
      async getsearlist(req,res,next){
         try {
            const ItemDetails = await db.RecentSearches.findAll({
             include: [{ model: db.customerModel,include:[{model:db.mapcustomeradress}]}]
            })
            res.status(200).send({
                status: 200,
    ...
    async update(req, res, next) {
        try {
           const { name, description, photo, tags, productId ,originPlace, aboutProduct, nutrientsDetals,storeUses,lableType,reccomendedProduct, isTex,GSTrate,GSTtyp, HSNcode, categoryId, subCategoryId,  videoUpload,} = req.body;
            ...
            db.product.findOne({ where: { id: productId },
                ...
                if (product) {
                   db.product.update({
                        ...
                        categoryId:categoryId,
                        subCategoryId:subCategoryId,
                        videoUpload:videoUpload }, { where: { id: productId } 
                    })
                }
                if(varientDetails ) {
                     var alldata = product
  ...
  async productOfferDelete(req, res, next) {
    ...
    async deleteRecomended(req,res,next){
        console.log(req.body)
        try{
            db.reccomendProduct.destroy({
                   where: {[Op.and]: [{ reccomendedId: {  [Op.like]: JSON.parse(req.body.reccomendedId) }}, { productId:  {  [Op.like]: req.body.pid}}]}})
                res.status(200).json({messag:"data  deleted successfull"})
         } catch (err) {
            next(err)
    ...
    //delete tags
    deletetedittags:(req,res,next) =>{
        try{
            db.tagModel.destroy({
                   where: {[Op.and]: [{ Name: {  [Op.like]: req.body.tagName }}, { productId:  {  [Op.like]: req.body.id}}]}})
                res.status(200).json({messag:"data  deleted successfull"})
        } catch (err) {
            next(err)
    ...
    //max less then price filter
    productPrizefilter:async (req,res,next) =>{
         try {
           await db.product.findAll({
                //   order: [['createdAt', 'DESC']],
                include: [{model:db.featureSettings,attributes:["recomendedProduct","deliverySlotStatus","loyaltyProgram","COD","showCOD","reccemendProductNo"]},{ model: db.productphoto, attributes: ["id", "imgUrl"] },{ model: db.varientModel, attributes: ["id", "sort","sku","waightunitno","unit","mrp","discount","price","stock","minstock","outofstock"]},
                ...
                let getlist =await product;
                let filterdata = getlist.filter((ele)=>{
                    return ele.varientModels[0].price < req.body.price ||ele.varientModels[1].price < req.body.price ||ele.varientModels[2].price < req.body.price || ele.varientModels[3].price < req.body.price;
                })
 ...
 //discount filter
    productDiscountfilter:async (req,res,next) =>{
         try {
           await db.product.findAll({
                //   order: [['createdAt', 'DESC']],
                include: [{model:db.featureSettings,attributes:["recomendedProduct","deliverySlotStatus","loyaltyProgram","COD","showCOD","reccemendProductNo"]},{ model: db.productphoto, attributes: ["id", "imgUrl"] },{ model: db.varientModel, attributes: ["id", "sort","sku","waightunitno","unit","mrp","discount","price","stock","minstock","outofstock"]},
                ...
                let getlist =await product;
                let filterdata = getlist.filter((ele)=>{
                    return ele.varientModels[0].discount < req.body.discount ||ele.varientModels[1].discount < req.body.discount ||ele.varientModels[2].discount < req.body.discount || ele.varientModels[3].discount < req.body.price;
                })
