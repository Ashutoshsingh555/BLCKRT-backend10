...
import factor from"../../../../2factorSmsService"
...
export default {
    ...
    async addalertMsgModel(req, res, next) {
        ...
        try {
            ...
            // await twilioService.sendAlertSms(clients, alerttext,message);
            const alertDetails = await db.alertMsgModel.create({
                ...
                for(let i=0; i<clients.length; i++){
                     await factor.sendMsgBySms(clients[i], alerttext,message);
                     db.alertClientNo.create({
                         alertId:clientslist.id,
                         clientNo:clients[i]
                    })
...
async getalert(req, res) {
    ...
    async getalertall(req, res){
        ...
        const ItemDetails = await db.alertMsgModel.findAll({ order: [['createdAt', 'DESC']],
                include: [{ model: db.alertClientNo }]
             })
            res.status(200).send({
                status: 200,
                message: 'Data fetched Successfully',
