...
export default {
    ...
    async adddeliveryAreasSettings(req, res, next) {
        ...
        try {
            const { city,  Zone, DeliveryAreaName, MinimumOrderAmount, ShippingFee,allowcustomers,ChargeShipping, AdditionalNote}= req.body;
...
async getdeliveryAreasSettings(req, res) {
   ...
   // get single  by city name
async getdeliveryAreasSettingsbycity(req, res) {
    try {
        const deliveryAreasSettingsDetails = await db.deliveryAreasSettings.findAll({ where: { city: req.body.city } });
        res.status(200).send({
            status: 200,
            message: 'Data fetched Successfully',
            data: deliveryAreasSettingsDetails
    ...
    async updatedeliveryAreasSettings(req, res){
        try {
            const { city,  Zone, DeliveryAreaName,areaId, MinimumOrderAmount, ShippingFee,allowcustomers,ChargeShipping, AdditionalNote}= req.body;
